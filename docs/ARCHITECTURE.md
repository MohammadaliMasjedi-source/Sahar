...
(`E:\Mo Pers\Pantheon\docs\ARCHITECTURE-STANDARD.md`). EMotion 3 is C#/Avalonia; Sahar is a web PWA,
...
## 6. Spaced repetition — the Leitner box (mirrors Pantheon)

Sahar reuses the **exact box model** proven in `E:\Mo Pers\Pantheon\leitner.html` so the family of
apps shares one learning engine:

- **5 boxes**, intervals **`[1, 2, 4, 9, 21]` days** (`INTERVALS` in `app.js`, copied from Pantheon).
