...
- **Q:** Have the tests actually been run? **A:** Honest note: the suite has been reviewed line-by-line and 28 well-formed `test(...)` cases were counted that assert against the real exports of `app.js`, but execution has not been confirmed in every environment (`node` is not always on PATH). Run `npm test` (i.e. `node test/core.test.js`) locally to confirm.
